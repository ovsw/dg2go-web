...
const trim = require("./src/filters/trim.js");
...
module.exports = (config) => {
  ...
  config.addFilter("blocksToMarkdown", function (sanityBlockContent) {
    return BlocksToMarkdown(sanityBlockContent, {
      serializers,
      ...client.config(),
    });
  });
  ...
  config.addShortcode("heroResponsiveBgImage", (image) => {
    const mobileUrl = urlFor(image).width(400).height(400).auto("format");
    const desktopUrl = urlFor(image).width(1600).height(774).auto("format");

    return `<style>.hero {background-image: url('${mobileUrl}')}
      @media screen and (min-width: 1600px) {.hero {background-image: url('${desktopUrl}')}}</style>`;
  });

  config.addShortcode(
    "pageHeaderResponsiveBgImage",
    (image, mobileW = 400, mobileH = 400, deskW = 1600, deskH = 774) => {
      ...
      return `<style>.page-header {background-image: url('${mobileUrl}')}
